...
- `src/sanopy/cli/cli.py` — click entrypoint (`sanopy` / `python -m sanopy`). Two commands: `init`, `scan`. `scan` exits `0` clean, `1` when findings exist, and `2` when it could not run at all: missing or invalid config, a missing linter, filters selecting no linters, or an unexpected error. `init` exits `2` if an install fails. Multi-target scans share one `asyncio.Semaphore` (governed by `CONCURRENT_SUBPROCESSES`, default 10) to cap concurrent linter subprocesses, and `_merge_payloads` folds each target's JSON into one document with `run.active_linters` set to the order-preserving union across targets.
...
- `src/sanopy/cli/init_handler.py` — `init` flow (interactive prompts + CLI options). Catches `FileNotFoundError`, `ValueError`, and `OSError` from `Config.load()` and falls back to `Config.defaults()` in memory, warning on the unreadable/invalid-file cases because saving overwrites it. Saves once after user confirms, then installs missing linters (prompted when interactive, automatic otherwise, never with `--no-install`) and exits `2` if any install fails.
- `src/sanopy/config.py` — `Config` dataclass, `load()` (read-only, raises on missing/invalid), `save()` (write-only, atomically: writes to a temp file in the destination directory then `Path.replace`s it over the target), `exists()`, `defaults()`, `LinterConfig`, `DEFAULT_LINTER_CONFIGS` (pylint/bandit/ruff), `DEFAULT_IGNORED_CVES` / `DEFAULT_IGNORED_VULNS`. Values are normalized (strip, dedupe; CVEs/vulns uppercased).
...
- `src/sanopy/linters/engine.py` — runs linters concurrently via asyncio. `run_all` accepts an optional `semaphore` (an `asyncio.Semaphore` shared across targets) to cap concurrent linter subprocesses; without it, all linters run at once.
...
- `Config.load()` is **read-only** — it never creates or modifies files. Missing file → `FileNotFoundError`. Invalid TOML → `ValueError`. OS-level read failures (permission/disk) → propagate as their native `OSError`, never masked as `ValueError`. The `init_handler` catches all three and creates defaults in memory; the scan handler exits `2`.
